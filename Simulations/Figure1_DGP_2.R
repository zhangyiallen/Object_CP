...
MAR_MTS_A <- function(n, reptime, rho, p, sigma_cross,d1){
  ...
  for(index in 1:reptime){
    ...
    ts_sim[[index]] <- cbind(t(inter[(burnin+1):(n/2+burnin),]),apply(t(inter[(burnin+n/2+1):(n+burnin),]),MARGIN = 2,function(X) X*c(((0.4+d1)/0.4),rep(1,p-1))))
  }
  return(ts_sim)
}
...
sigma_cross = 0.4*sqrt(1-rho^2)*diag(c(1,1))
p=2
...
write.csv(resultrawpl,paste0("./result/2dim_mod2_dist_SSSN2_power_SSSN_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
write.csv(result1,paste0("./result/2dim_mod2_dist_JiangZhuShao_SN1_power_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
write.csv(result2,paste0("./result/2dim_mod2_dist_JiangZhuShao_SN2_power_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
write.csv(resultDM,paste0("./result/2dim_mod2_dist_DM_power_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
write.csv(resultDM23,paste0("./result/2dim_mod2_dist_DM23_power_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
...
for (dt in d_sets) {
  ...
  for (ite in 1:40) {
    ...
    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod2_dist_SSSN2_power_SSSN_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    tempdata1=c(tempdata1,temp)

    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod2_dist_JiangZhuShao_SN1_power_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    tempdata2=c(tempdata2,temp)

    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod2_dist_JiangZhuShao_SN2_power_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    tempdata3=c(tempdata3,temp)

    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod2_dist_DM_power_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    tempdata4=c(tempdata4,temp)

    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod2_dist_DM23_power_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
...
critSN1 = 5.775127 ##SSSN
critSN2 = 53.53165##sn1
critSN3 = 27.88118#SN2
critSN4 = 0.00000#DM
critSN5 = 0.000 ###DM23
#####my power critic : rho=0
# critSN1 = 5.163318 ##SSSN
# critSN2 = 47.01051##sn1
# critSN3 = 29.84941#SN2
# critSN4 = 0.085#DM
# critSN5 = 0.045 ###DM23
#####my power critic : rho=-0.4
critSN1 = 5.189803 ##SSSN
critSN2 = 48.30429##sn1
critSN3 = 39.26892#SN2
critSN4 = 0.035#DM
critSN5 = 0.045 ###DM23
