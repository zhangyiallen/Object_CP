...
rho = as.numeric(command.args[1])##-0.4,0,0.4,0.7
...
write.csv(resultrawpl,paste0("./result/2dim_mod1_dist_SSSN2_size_SSSN_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
write.csv(result1,paste0("./result/2dim_mod1_dist_JiangZhuShao_SN1_size_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
write.csv(result2,paste0("./result/2dim_mod1_dist_JiangZhuShao_SN2_size_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
write.csv(resultDM,paste0("./result/2dim_mod1_dist_DM_size_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
write.csv(resultDM23,paste0("./result/2dim_mod1_dist_DM23_size_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),row.names = F)
...
for (n in c(400)) {
  for (rho in c(-0.4,0,0.4,0.7)){
    ...
    temp5 = c(n,rho,dt)
    for (ite in 1:40) {
    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod1_dist_SSSN2_size_SSSN_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    temp1=c(temp1,temp)

    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod1_dist_JiangZhuShao_SN1_size_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    temp2=c(temp2,temp)

    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod1_dist_JiangZhuShao_SN2_size_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    temp3=c(temp3,temp)

    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod1_dist_DM_size_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    temp4=c(temp4,temp)

    temp=read.csv(file=paste0("./result/2dim_dist/2dim_mod1_dist_DM23_size_n",n,"_rho",rho,"_dt",dt,"_ite",ite,"_v1.csv"),header = F)
    temp=as.numeric(temp[2:length(temp[,1]),4:28])
    temp5=c(temp5,temp)
    }
    data1=rbind(data1,temp1)
    data2=rbind(data2,temp2)
    data3=rbind(data3,temp3)
    data4=rbind(data4,temp4)
    data5=rbind(data5,temp5)
    }
}
Esizen1 = c()
Esizen2 = c()
...
for (i in 1:length(data1[,1])) {
  ...
  r95 = sum(as.numeric(data1[i,4:length(data1[i,])])>q95sssn)/reptime
  ...
  Esizen1=rbind(Esizen1,c(as.numeric(data1[i,1]),as.numeric(data1[i,2]),as.numeric(data1[i,3]),
                          quantile(as.numeric(data1[i,4:length(data1[i,])]),0.95),r95))

  r95 = sum(as.numeric(data2[i,4:length(data2[i,])])>q95)/reptime
  #prop1 = sum(as.numeric(Jstar[i,5:length(Jstar)])==as.numeric(Jstar[i,2]))/reptime ##last dimention
  #prop1 = sum(as.numeric(Jstar[i,5:length(Jstar)])==1)/reptime ###fixed dimention
  Esizen2=rbind(Esizen2,c(as.numeric(data2[i,1]),as.numeric(data2[i,2]),as.numeric(data2[i,3]),
                          quantile(as.numeric(data2[i,4:length(data2[i,])]),0.95),r95))

  r95 = sum(as.numeric(data3[i,4:length(data3[i,])])>q95)/reptime
  #prop1 = sum(as.numeric(Jstar[i,5:length(Jstar)])==as.numeric(Jstar[i,2]))/reptime ##last dimention
  #prop1 = sum(as.numeric(Jstar[i,5:length(Jstar)])==1)/reptime ###fixed dimention
  Esizen3=rbind(Esizen3,c(as.numeric(data3[i,1]),as.numeric(data3[i,2]),as.numeric(data3[i,3]),
                          quantile(as.numeric(data3[i,4:length(data3[i,])]),0.95),r95))

  r95 = sum(as.numeric(data4[i,4:length(data4[i,])])<=0.05)/reptime
  #prop1 = sum(as.numeric(Jstar[i,5:length(Jstar)])==as.numeric(Jstar[i,2]))/reptime ##last dimention
  #prop1 = sum(as.numeric(Jstar[i,5:length(Jstar)])==1)/reptime ###fixed dimention
  Esizen4=rbind(Esizen4,c(as.numeric(data4[i,1]),as.numeric(data4[i,2]),as.numeric(data4[i,3]),
                          quantile(as.numeric(data4[i,4:length(data4[i,])]),0.05),r95))

  r95 = sum(as.numeric(data5[i,4:length(data5[i,])])<=0.05)/reptime
  #prop1 = sum(as.numeric(Jstar[i,5:length(Jstar)])==as.numeric(Jstar[i,2]))/reptime ##last dimention
  #prop1 = sum(as.numeric(Jstar[i,5:length(Jstar)])==1)/reptime ###fixed dimention
  Esizen5=rbind(Esizen5,c(as.numeric(data5[i,1]),as.numeric(data5[i,2]),as.numeric(data5[i,3]),
                          quantile(as.numeric(data5[i,4:length(data5[i,])]),0.05),r95))
}
